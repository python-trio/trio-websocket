...
class fail_after:
    ...
    def __call__(
        self, fn: Callable[PS, Awaitable[T]]
    ) -> Callable[PS, Awaitable[T | None]]:
...
async def test_client_open_url(
    path: str, expected_path: str, echo_server: WebSocketServer
) -> None:
...
async def test_client_connect(
    echo_server: WebSocketServer, nursery: trio.Nursery
) -> None:
...
async def test_client_connect_url(
    echo_server: WebSocketServer, nursery: trio.Nursery
) -> None:
    url = f"ws://{HOST}:{echo_server.port}{RESOURCE}"
...
async def test_server_close_timeout(autojump_clock: trio.testing.MockClock) -> None:
    ...
    async with trio.open_nursery() as outer:
        ...
        async with trio.open_nursery() as inner:
            await connect_websocket(inner, HOST, server.port, RESOURCE, use_ssl=False)
...
@fail_after(DEFAULT_TEST_MAX_DURATION)
async def test_server_tcp_closed_on_close_connection_event(
    nursery: trio.Nursery,
) -> None:
...
def test_copy_exceptions() -> None:
    ...
    assert (
        copy.copy(ConnectionClosed("foo")).reason  # type: ignore[arg-type]
        == "foo"  # type: ignore[comparison-overlap]
    )
